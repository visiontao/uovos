...
public:
  ...
  virtual bool saveImagePGM(const char* filename, T scale = 1, T shift = 0) const;
  virtual bool saveImagePGM(ofstream& myfile, T scale = 1, T shift = 0) const;
#ifndef _MATLAB
...
template <class T>
bool Image<T>::saveImagePGM(const char* filename, T scale, T shift) const {
  ofstream myfile(filename, ios::out | ios::binary);
  bool ret = false;
  if (myfile.is_open()) {
    ret = saveImagePGM(myfile, scale, shift);
    myfile.close();
  }
  return ret;
}
...
template <class T>
bool Image<T>::saveImagePGM(ofstream& myfile, T scale, T shift) const {
  if (nChannels == 1) {
    myfile << "P5\n" << imWidth << " " << imHeight << "\n" << 255 << "\n";
  } else if (nChannels == 3) {
    myfile << "P6\n" << imWidth << " " << imHeight << "\n" << 255 << "\n";
  ...
  for (int i = 0; i < imWidth * imHeight * nChannels; ++i) {
    T v = pData[i] * scale + shift;
    unsigned char c = static_cast<unsigned char>(v < 0 ? 0 : v > 255 ? 255 : v);
    myfile.write((char*)&c, sizeof(unsigned char));
  }
  return true;
}
...
template <class T>
template <class T1>
void Image<T>::GaussianSmoothing(Image<T1>& image,
                                 double sigma, int fsize) const {
  Image<T1> foo;
...
template <class T>
template <class T1>
void Image<T>::imfilter(Image<T1>& image,
                        const double* filter, int fsize) const {
  if (matchDimension(image) == false)
    image.allocate(imWidth, imHeight, nChannels);
  ImageProcessing::filtering(pData, image.data(),
                             imWidth, imHeight, nChannels, filter, fsize);
}
...
void Image<T>::imfilter(Image<T1>& image, const Image<T2>& kernel) const {
  if (kernel.width()!=kernel.height()) {
    cout << "Error in Image<T>::imfilter(Image<T1>& image, "
        "const Image<T2>& kernel)" << endl;
    exit(-1);
  }
...
void Image<T>::imfilter_h(Image<T1>& image, double* filter, int fsize) const {
  if (matchDimension(image)==false)
    image.allocate(imWidth, imHeight, nChannels);
  ImageProcessing::hfiltering(pData, image.data(),
                              imWidth, imHeight, nChannels, filter, fsize);
}
...
void Image<T>::imfilter_v(Image<T1>& image, double* filter, int fsize) const {
  if (matchDimension(image)==false)
    image.allocate(imWidth, imHeight, nChannels);
  ImageProcessing::vfiltering(pData, image.data(),
                              imWidth, imHeight, nChannels, filter, fsize);
}
...
template <class T>
template <class T1>
void Image<T>::imfilter_hv(Image<T1> &image, 
                           const double *hfilter, int hfsize, 
  ...
  ImageProcessing::hfiltering(pData, pTempBuffer,
                              imWidth, imHeight, nChannels, hfilter, hfsize);
  ImageProcessing::vfiltering(pTempBuffer, image.data(),
                              imWidth, imHeight, nChannels, vfilter, vfsize);
  delete pTempBuffer;
}
